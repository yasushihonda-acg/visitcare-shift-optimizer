...
| サービス種別マスタ | `/masters/service-types` | サービス種別の一覧・編集 |
...
### 7.4 サービス種別マスタ (`/masters/service-types`)

サービス種別（身体介護、生活援助、介護予防など）のマスタを管理します。

**一覧表示**:
- コード、名称、サービスカテゴリ（身体/生活/予防/その他）が表示
- 管理者・サ責のみ閲覧可能
...
> **注意**: サービス種別はシフト最適化エンジンの制約判定（資格要否）に影響します。変更時は最適化結果への影響を確認してください。
